...
def refresh_ui(target=None):
    """מרענן את תצוגת TradingView (Page.navigate — בדיוק כמו ב-clean_list הישן) כדי שכתיבות REST
    יוצגו בחלון. אם target ניתן — עובר אליו כדי שהמשתמש יראה את הרשימה המעודכנת."""
    try:
        pages = json.loads(urllib.request.urlopen('http://localhost:9222/json').read())
        page = next(p for p in pages if 'tradingview.com' in p.get('url', ''))
        ws = websocket.create_connection(page['webSocketDebuggerUrl'], timeout=20, header={'Origin': 'http://localhost'})
    except Exception as e:
        print(f"  refresh_ui: לא ניתן להתחבר ({e})")
        return
    mid = [1]
    ws.send(json.dumps({"id": mid[0], "method": "Page.enable", "params": {}})); mid[0] += 1
    try:
        ws.recv()
    except Exception:
        pass
    m = mid[0]; mid[0] += 1
    ws.send(json.dumps({"id": m, "method": "Page.navigate", "params": {"url": "https://www.tradingview.com/chart/"}}))
    for _ in range(30):
        try:
            r = json.loads(ws.recv())
            if r.get('id') == m:
    ...
    if target:
        try:
            switch_to_dayscan(ws, target=target)
        except Exception as e:
            print(f"  refresh_ui switch: {e}")
    ws.close()
    print(f"  ✅ תצוגת TradingView רועננה{' → ' + target if target else ''}")
